...
void Time::time(Client* client, std::vector<std::string> args, Server* srv)
{
	if (srv == NULL)
...
void List::list( Client* client, std::vector<std::string> args, Server* srv)
{
	if (srv == NULL)
		return (client->sendReply(ERR_NOSUCHSERVER(client->getNick(), client->getHostname())));
	...
	while (it != channels.end())
	{
		if (!it->second->isPrivateChannel())
		{
			...
			std::string channelInfo = channel->getChannelInfo();
			// topic alanı yapılacak
			client->sendReply(RPL_LIST(client->getNick(), channelName, channel->getChannelClientCount(), "No topic"));
		}
		it++;
	}
	client->sendMsg(":" + client->getHostname() + "323 " + client->getNick() + " :End of /LIST");
}
...
void Topic::topic( Client* client, std::vector<std::string> args, Server* srv )
{
	...
	if (topic == "" && srv->getChannel(channelName)->getTopic() != "")
	{
		client->sendReply(RPL_TOPIC(client->getNick(), channelName, srv->getChannel(channelName)->getTopic()));
		client->sendReply(RPL_TOPICWHOTIME(client->getNick(), channelName, srv->getChannel(channelName)->getCreator()->getNick(), srv->getChannel(channelName)->getTopicTime()));
		return;
	}
	if (topic == ":")
		topic = "";
	...
	if (topic.empty())
	{
		client->sendReply(RPL_TOPIC(client->getNick(), channelName, channel->getTopic()));
		client->sendReply(RPL_TOPICWHOTIME(client->getNick(), channelName, client->getNick(), channel->getTopicTime()));
	}
	else
	{
		if (!client->isModeratorStatus())
